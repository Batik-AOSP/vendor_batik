...
func init() {
	android.RegisterModuleType("batik_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = batikExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = batikExpandVariables(ctx, dep_file)
	...
	cmd := batikExpandVariables(ctx, String(g.properties.Cmd))
